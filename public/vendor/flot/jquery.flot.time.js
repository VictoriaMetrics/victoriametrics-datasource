...
(function($) {
	...
	function makeUtcWrapper(d) {
		...
		let props = ["Date", "Day", "FullYear", "Hours", "Milliseconds", "Minutes", "Month", "Seconds"];
			...
			$.each(plot.getAxes(), function(axisName, axis) {
				...
				if (opts.mode == "time") {
					axis.tickGenerator = function(axis) {
						...
						let ticks = [];
						let d = dateGenerator(axis.min, opts);
						let minSize = 0;
					...
					axis.tickFormatter = function (v, axis) {
						...
						let d = dateGenerator(v, axis.options);
						...
						let useQuarters = (axis.options.tickSize &&
						...
						let suffix = (opts.twelveHourClock) ? " %p" : "";
						let hourCode = (opts.twelveHourClock) ? "%I" : "%H";
						let fmt;
						...
						let rt = formatDate(d, fmt, opts.monthNames, opts.dayNames);
