...
(function($) {
    ...
    let Gauge = (function() {
        ...
        Gauge.prototype.calculateLayout = function() {
            ...
            let canvasWidth = placeholder.width();
            let canvasHeight = placeholder.height();



            // calculate cell size
            let columns = Math.min(series.length, gaugeOptions.layout.columns);
            let rows = Math.ceil(series.length / columns);



            let margin = gaugeOptions.layout.margin;
            let hMargin = gaugeOptions.layout.hMargin;
            let vMargin = gaugeOptions.layout.vMargin;
            let cellWidth = (canvasWidth - (margin * 2) - (hMargin * (columns - 1))) / columns;
            let cellHeight = (canvasHeight - (margin * 2) - (vMargin * (rows - 1))) / rows;
            ...
            let cellMargin = gaugeOptions.cell.margin;
            let labelMargin = 0;
            let labelFontSize = 0;
            ...
            let thresholdLabelMargin = 0;
            let thresholdLabelFontSize = 0;
            ...
            let maxRadiusH = (cellWidth / 2) - cellMargin - thresholdWidth - (thresholdLabelMargin * 2) - thresholdLabelFontSize;

            let startAngle = gaugeOptions.gauge.startAngle;
            let endAngle = gaugeOptions.gauge.endAngle;
            let dAngle = (endAngle - startAngle) / 100;
            let heightRatioV = -1;
            for (let a = startAngle; a < endAngle; a += dAngle) {
                heightRatioV = Math.max(heightRatioV, Math.sin(toRad(a)));
            }
            heightRatioV = Math.max(heightRatioV, Math.sin(toRad(endAngle)));
            let outerRadiusV = (cellHeight - (cellMargin * 2) - (labelMargin * 2) - labelFontSize) / (1 + heightRatioV);
            ...
            let maxRadiusV = outerRadiusV - (thresholdLabelMargin * 2) - thresholdLabelFontSize - thresholdWidth;

            let radius = Math.min(maxRadiusH, maxRadiusV);


            let width = gaugeOptions.gauge.width;
            if (width >= radius) {
                width = Math.max(3, radius / 3);
            }


            let outerRadius = (thresholdLabelMargin * 2) + thresholdLabelFontSize + thresholdWidth + radius;
            let gaugeOuterHeight = Math.max(outerRadius * (1 + heightRatioV), outerRadius + valueMargin + (valueFontSize / 2));
        ...
        Gauge.prototype.calculateCellLayout = function(gaugeOptionsi, layout, i) {
            ...
            let c = col(layout.columns, i);
            let r = row(layout.columns, i);
            let x = layout.margin + (layout.cellWidth + layout.hMargin) * c;
            let y = layout.margin + (layout.cellHeight + layout.vMargin) * r;
            let cx = x + (layout.cellWidth / 2);
            let cy = y + layout.cellMargin + (layout.labelMargin * 2) + layout.labelFontSize + layout.thresholdWidth
                        + layout.thresholdLabelFontSize + (layout.thresholdLabelMargin * 2) + layout.radius;
            let blank = layout.cellHeight - (layout.cellMargin * 2) - (layout.labelMargin * 2) - layout.labelFontSize - layout.gaugeOuterHeight;
            let offsetY = 0;
        ...
        Gauge.prototype.drawGauge = function(gaugeOptionsi, layout, cellLayout, label, data) {
            ...
            let blur = gaugeOptionsi.gauge.shadow.show ? gaugeOptionsi.gauge.shadow.blur : 0;
            ...
            // draw gauge
            let c1 = getColor(gaugeOptionsi, data);
            let a2 = calculateAngle(gaugeOptionsi, layout, data);
            drawArcWithShadow(
                cellLayout.cx, // center x
                cellLayout.cy, // center y
                layout.radius - 1,
        ...
        function getColor(gaugeOptionsi, data) {
            let color;
            for (let i = 0; i < gaugeOptionsi.threshold.values.length; i++) {
                let threshold = gaugeOptionsi.threshold.values[i];
        ...
        Gauge.prototype.drawThreshold = function(gaugeOptionsi, layout, cellLayout) {
            ...
            let a1 = gaugeOptionsi.gauge.startAngle;
            for (let i = 0; i < gaugeOptionsi.threshold.values.length; i++) {
                let threshold = gaugeOptionsi.threshold.values[i];
                c1 = threshold.color;
        ...
        Gauge.prototype.drawThresholdValues = function(gaugeOptionsi, layout, cellLayout, i) {
            ...
            // threshold values
            for (let j = 0; j < gaugeOptionsi.threshold.values.length; j++) {
                let threshold = gaugeOptionsi.threshold.values[j];
                if (threshold.value > gaugeOptionsi.gauge.min && threshold.value < gaugeOptionsi.gauge.max) {
                    let a = calculateAngle(gaugeOptionsi, layout, threshold.value);
        ...
        function drawText(x, y, id, text, textOptions, a) {
            let span = $(placeholder).find("#" + id);
            let exists = span.length;
            if (!exists) {
                span = $("<span></span>")
                span.attr("id", id);
    ...
    function init (plot) {
        ...
        plot.hooks.processOptions.push(function(plot, options) {
            let logger = getLogger(options.series.gauges.debug);
            ...
            // sort threshold
            let thresholds = options.series.gauges.threshold.values;
        ...
        plot.hooks.draw.push(function(plot, context) {
            ...
            let gaugeOptions = options.series.gauges;

            let logger = getLogger(gaugeOptions.debug);
            ...
            let gauge = new Gauge(plot, context);

            // calculate layout
            let layout = gauge.calculateLayout();
            ...
            for (let i = 0; i < series.length; i++) {
                ...
                let gaugeOptionsi = $.extend({}, gaugeOptions, item.gauges);
                if (item.gauges) {
                ...
                let cellLayout = gauge.calculateCellLayout(gaugeOptionsi, layout, i);
